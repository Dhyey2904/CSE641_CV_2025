# -*- coding: utf-8 -*-
"""efficientnetb3-rstp.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1fNxLoU2mxFiHSuZzUuHbtgvIArnpBQXP
"""
...
df = pd.read_csv("/kaggle/input/rstp-caption/reformatted_captions.csv")
df.columns = ['image', 'caption_number', 'caption', 'id']

# Strip leading spaces from caption only
df['caption'] = df['caption'].str.lstrip()

# Add or modify the last row
df.loc[19999, 'caption_number'] = 4
df.loc[19999, 'caption'] = "A dog runs across the grass."

# Optional: Recompute IDs (if needed)
...
df['id'] = ids
df.to_csv("captions1.csv", index=False)

df = pd.read_csv("/kaggle/working/captions.csv", delimiter=",")
print(df.columns)
df.head(10)

df = pd.read_csv("/kaggle/working/captions1.csv", delimiter=",")
print(df.columns)
df.head(10)
...
class CFG:
    debug = False
    image_path = "/kaggle/input/rstpreid-1/imgs"
...
for epoch in range(cfg.epochs):
    ...
    # Early Stopping Condition
    if early_stopping_counter >= early_stopping_patience:
        print(f"⏹️ Early stopping triggered! No improvement for {early_stopping_patience} epochs.")
        break  # Stop training
...
model.load_state_dict(torch.load("/kaggle/input/rstp-efficientnet/best_efficientNetB3.pt", map_location=device))
model.eval()
...
model, image_embeddings = get_image_embeddings(valid_df, "/kaggle/input/rstp-efficientnet/best_efficientNetB3.pt")
...
find_matches(model,
             image_embeddings,
             query="The man is walking.He is wearing a grey jacket and black trousers.His shoes are brown and his bag is black.He is looking around.",
