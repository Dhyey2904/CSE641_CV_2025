# -*- coding: utf-8 -*-
"""notebook228c511a12.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1QGTaVV7pzLESZLNry0VACv3tBR9D5Z5q
...
class ImageEncoder(nn.Module):
    def __init__(self, embed_dim=cfg.image_embedding):
        ...
        self.attnpool = AttentionPool2d(spacial_dim=7, embed_dim=1280, num_heads=8, output_dim=embed_dim)
        std = 1280 ** -0.5
...
model = CLIPModel().to(device)  # Matches your training architecture
model.load_state_dict(torch.load("/kaggle/input/model/pytorch/default/1/best(4).pt", map_location=device))  # Load trained weights
model.eval()  # Set to evaluation mode

# Step 2: Load validation dataset and tokenizer
tokenizer = DistilBertTokenizer.from_pretrained(CFG.text_tokenizer)
_, valid_df = make_train_valid_dfs()  # Get validation split
valid_loader = build_loaders(valid_df, tokenizer, mode="valid")
...
image_features = extract_features(valid_loader, model, "image")  # Shape: (num_unique_images, 256)
text_features = extract_features(valid_loader, model, "text")    # Shape: (num_captions, 256)

# Step 4: Prepare data for your metrics function
csv_df = valid_df  # Your captions DataFrame
unique_df = valid_df.drop_duplicates(subset=["image"]).reset_index(drop=True)  # Unique images
text_embeddings = text_features  # Already normalized
unique_image_embeddings = image_features  # Already normalized

# Step 5: Your new retrieval metrics function (renamed)
def compute_rank_metrics(csv_df, text_embeddings, unique_image_embeddings, unique_df):
    """
    Computes Rank@1, Rank@5, and Rank@10 metrics for text-to-image retrieval.
    """
    # Create a mapping from image_name to index in the unique image embeddings tensor
    id_to_index = {row['image']: idx for idx, row in unique_df.iterrows()}

    # Compute similarity matrix
    similarity_matrix = text_embeddings @ unique_image_embeddings.T  # Shape: (num_captions, num_unique_images)
    num_queries = similarity_matrix.shape[0]

    # Sort image indices by similarity score
    ...
    for i in tqdm(range(num_queries), desc="Computing retrieval ranks"):
        gt_image = csv_df.iloc[i]['image']  # Adjusted to match your column name
        gt_index = id_to_index[gt_image]
        ranking = sorted_indices[i]
        rank = (ranking == gt_index).nonzero(as_tuple=True)[0].item() + 1
    ...
    rank1 = rank1_count / num_queries
    rank5 = rank5_count / num_queries
    rank10 = rank10_count / num_queries
...
# Step 6: Compute and print your new metrics
print("\nYour Rank Metrics:")
rank1, rank5, rank10 = compute_rank_metrics(csv_df, text_embeddings, unique_image_embeddings, unique_df)

# Step 7: Original metrics for comparison
similarity_matrix = np.matmul(text_features.cpu().numpy(), image_features.cpu().numpy().T)

# Original function from your notebook (unchanged)
...
print("\nOriginal Retrieval Performance:")
for k in recall_k:
...
# # Step 1: Initialize the CLIP Model
# model = CLIPModel().to(device)  # Matches your training architecture
# model.load_state_dict(torch.load("/kaggle/working/best.pt", map_location=device))  # Load trained weights
# model.eval()  # Set to evaluation mode
...
# # Step 1: Initialize the CLIP Model
# model = CLIPModel().to(device)  # Matches your training architecture
# model.load_state_dict(torch.load("/kaggle/input/mobilenetv2/pytorch/default/1/best.pt", map_location=device))  # Load trained weights
# model.eval()  # Set to evaluation mode
...
# # Step 1: Initialize the CLIP Model
# model = CLIPModel().to(device)  # Matches your training architecture
# model.load_state_dict(torch.load("/kaggle/input/mobilenet_v2_1/pytorch/default/1/best (1).pt", map_location=device))  # Load trained weights
# model.eval()  # Set to evaluation mode
